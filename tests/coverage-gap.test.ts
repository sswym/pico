...
// ── undo-redo ────────────────────────────────────────────────────────────
import { replaceRootInText } from "../src/extensions/undo-redo/paths.ts";
import { formatDiffText, listDiffItems, showDiffStack } from "../src/extensions/undo-redo/diff-stack.ts";
import { createCache } from "../src/extensions/undo-redo/cache.ts";
import { createBufferedToolSet, registerBufferedTools } from "../src/extensions/undo-redo/tools.ts";
...
describe("undo-redo paths/diff/tools", () => {
  test("replaceRootInText rewrites sandbox root in output", () => {
    expect(
      replaceRootInText("file at /sbox/x.txt", "/sbox", "/real"),
    ).toBe("file at /real/x.txt");
    // no match → unchanged
    expect(replaceRootInText("plain", "/sbox", "/real")).toBe("plain");
  });

  test("formatDiffText handles empty/binary/normal cases", async () => {
    const dir = mkdtempSync(join(tmpdir(), "pico-gap-cache-"));
    try {
      const cache = createCache("gap-test-session");
      await cache.ensure();
      expect(
        await formatDiffText(cache, undefined, undefined),
      ).toBe("No changes recorded.");

      const { hashBuffer } = await import("../src/extensions/undo-redo/cache.ts");
      const hashA = hashBuffer(Buffer.from("old"));
      const hashB = hashBuffer(Buffer.from("new"));
      await cache.writeBlob(hashA, Buffer.from("old"));
      await cache.writeBlob(hashB, Buffer.from("new"));
      const diff = await formatDiffText(
        cache,
        { exists: true, hash: hashA },
        { exists: true, hash: hashB },
      );
  ...
  test("showDiffStack no-UI path notifies error", async () => {
    const pi = {
      sendMessage: () => {},
    };
    let notify: Array<[string, string]> = [];
    const ctx = {
      hasUI: false,
      ui: { notify: (m: string, t: string) => notify.push([m, t]) },
    } as never;
    await showDiffStack(pi as never, ctx as never, {} as never, {} as never);
    expect(notify[0]).toEqual(["No UI available", "error"]);
  });

  test("createBufferedToolSet wires buffered tools", async () => {
    const dir = mkdtempSync(join(tmpdir(), "pico-gap-buf-"));
    try {
      const realRoot = join(dir, "real");
      const sandboxRoot = join(dir, "sandbox");
      mkdirSync(realRoot, { recursive: true });
      mkdirSync(sandboxRoot, { recursive: true });
      writeFileSync(join(realRoot, "a.txt"), "hello");
      const cache = createCache("gap-buffered-tools");
      await cache.ensure();
      ...
      expect(typeof set.readTool).toBe("object");
      expect(typeof set.editTool).toBe("object");
      expect(typeof set.bashTool).toBe("object");
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  });

  test("registerBufferedTools registers all seven tools", async () => {
    const dir = mkdtempSync(join(tmpdir(), "pico-gap-reg-"));
    try {
      const realRoot = join(dir, "real");
      const sandboxRoot = join(dir, "sandbox");
      mkdirSync(realRoot, { recursive: true });
      mkdirSync(sandboxRoot, { recursive: true });
      const cache = createCache("gap-register-tools");
      await cache.ensure();
      const tracker = new SnapshotTracker(cache, realRoot, sandboxRoot, () => {});
      const registered: string[] = [];
      registerBufferedTools({
        realRoot,
        sandboxRoot,
        tracker,
        sandboxState: {
  ...
  test("buffered read tool executes through sandbox mapping (mapInputPath/rewriteResultPaths)", async () => {
    const dir = mkdtempSync(join(tmpdir(), "pico-gap-read-"));
    try {
      const realRoot = join(dir, "real");
      const sandboxRoot = join(dir, "sandbox");
      mkdirSync(realRoot, { recursive: true });
      mkdirSync(sandboxRoot, { recursive: true });
      writeFileSync(join(realRoot, "a.txt"), "hello world");
      const cache = createCache("gap-read-tool");
      await cache.ensure();
      const tracker = new SnapshotTracker(cache, realRoot, sandboxRoot, () => {});
      const set = createBufferedToolSet({
        realRoot,
      ...
      // path is relative to realRoot; the tool maps it into the sandbox.
  ...
  test("buffered find tool maps path into sandbox (mapInputPath/rewriteError)", async () => {
    const dir = mkdtempSync(join(tmpdir(), "pico-gap-find-"));
    try {
      const realRoot = join(dir, "real");
      const sandboxRoot = join(dir, "sandbox");
      mkdirSync(realRoot, { recursive: true });
      mkdirSync(sandboxRoot, { recursive: true });
      writeFileSync(join(realRoot, "a.txt"), "hello world");
      const cache = createCache("gap-find-tool");
      await cache.ensure();
      const tracker = new SnapshotTracker(cache, realRoot, sandboxRoot, () => {});
      const set = createBufferedToolSet({
        realRoot,
      ...
      const ok = await set.findTool.execute(
        "t2",
        { path: realRoot, pattern: "hello" },
        undefined,
        () => {},
        {} as never,
      );
      const text =
        (ok.content as Array<{ type: string; text?: string }>)
          .find((c) => c.type === "text")?.text ?? "";
      // mapInputPath rewrote realRoot → sandboxRoot so the tool ran in the
      // sandbox. The file isn't there yet (find does not copy), so a clean
      // "no files" response still proves the mapping executed without error.
      expect(typeof text).toBe("string");
      expect(/No files|matching pattern/i.test(text)).toBe(true);
      // Nonexistent dir raises → rewriteError wraps the sandbox path.
      await expect(
        set.findTool.execute(
          "t3",
          { path: join(realRoot, "no-such-dir"), pattern: "x" },
          undefined,
          () => {},
          {} as never,
        ),
  ...
  test("listDiffItems maps describeChange/toChangeType across leaves", async () => {
    const dir = mkdtempSync(join(tmpdir(), "pico-gap-list-"));
    try {
      const realRoot = join(dir, "real");
      const sandboxRoot = join(dir, "sandbox");
      mkdirSync(realRoot, { recursive: true });
      mkdirSync(sandboxRoot, { recursive: true });
      const cache = createCache("gap-list-diff");
      await cache.ensure();
      const tracker = new SnapshotTracker(cache, realRoot, sandboxRoot, () => {});
      // base manifest: a.txt exists, b.txt deleted, c.txt missing (added)
      const { hashBuffer } = await import("../src/extensions/undo-redo/cache.ts");
