...
- `ccstyle`（`src/extensions/ccstyle/`）是移植自 https://github.com/minuque/pi-cc-extensions（MIT，v0.8.54）的内置扩展（phase: ui）：Claude Code 风格工具渲染——连续工具调用分组（`Container.prototype.addChild/removeChild/clear` 补丁 → `ToolGroupComponent`）、单行摘要 + 状态图标、Input/Output 展开视图、edit/write 结果自动展开与着色 diff（复用上游 `result.details.diff`，无 shiki 依赖）、fullscreen 鼠标点击展开/收起（实例级包装 `handleViewportInput` + `currentLayout` 布局树命中）；思考块折叠——原型补丁 `AssistantMessageComponent.prototype.updateContent`（`thinking.ts`），每条助手消息的思考序列默认一行 `Thinking… · click to expand` 标题、鼠标点击展开全文/再点收起（逐块独立，状态随组件重建丢失，与工具卡展开一致），`hideThinkingBlock`（ctrl+t）保留上游惰性标签语义、`/ccstyle off` 回退原生长文渲染。配置：settings.json `ccstyle.enabled`（默认 true）+ `/ccstyle on|off|status`。接管规则：**全部工具**统一 ccstyle 渲染（上游内置与 pico 定制工具），渲染与执行解耦；pico 工具折叠摘要复用 `summarizeToolCall`（`tool-render.ts`）。改这里前先读 `render.ts`（原型补丁的 downstream 必须是方法值快照，别名会自递归）、`grouping.ts`（edit/write/apply_patch 不入组但渲染被接管）与 `thinking.ts`（updateContent 补丁需同时复刻上游 text 分支的 markdown transform，mermaid 等 transformer 来自构造参数）
- `evolution`（`src/extensions/evolution/`）是内置扩展（phase: runtime）：自进化闭环——`agent_end` 回合末异步审查会话消息（辅助模型直调，`review.ts`），输出严格 JSON 经 `apply.ts` 校验（技能名消毒、路径穿越、注入特征 `PICO_EVOLUTION_DENY`、大小上限）后写入 `~/.pico/agent/skills/`，`.pico-evolved.json` 清单记录自产技能（用户手写技能永不触碰，mtime 比对新则跳过；条目含 `useCount/lastUsedAt/lastResult` 使用统计）。**使用反馈环**：`tool_result` 订阅捕获 `skill action=run`（成功/失败）计入清单统计，审查 prompt 的 existing 块注入使用统计（只统计显式 run，隐式注入使用不可观测）；`/evolution` 命令列出自产技能 + 统计 + `[stale]` 标记（30 天未用），`/doctor` Evolution 段同源展示。默认关（`evolution.enabled` / `PICO_EVOLUTION_ENABLED`）；设计文档 `docs/evolution-design.md`。改这里前先读 `apply.ts` 的安全校验清单与 `index.ts` 的触发/频率限制
