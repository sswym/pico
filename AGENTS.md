...
srcode 是 `@earendil-works/pi-coding-agent` 的 **thin wrapper**。上游提供 agent loop、tool runtime、session 管理；srcode 通过 19 个 ExtensionFactory 插件注入功能。
...
`vibe → oma → retro-theme → language → input-history → logo → memory → subagent → todo → vision → ask → init → plan → rtk → web → lsp → hooks → mcp → doctor`
