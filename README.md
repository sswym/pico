...
基于 `@earendil-works/pi-coding-agent` 的薄封装：上游提供 agent loop、工具运行时与会话管理，srcode 通过 19 个扩展注入产品化能力。技术栈：Bun + TypeScript，零外部运行时依赖。
...
首次使用建议运行 `srcode setup`（写入 `~/.srcode/agent/settings.json`，支持按 section 配置：`setup model` / `tools` / `safety` / `ui` / `--reset`）。
...
以下能力**默认关闭**，需显式开启（`~/.srcode/agent/settings.json` 的 `safety` 字段或环境变量，env 优先）：

- 项目级 shell hooks（`SRCODE_ENABLE_PROJECT_HOOKS`）
- 项目级 MCP 服务器（`SRCODE_ENABLE_PROJECT_MCP`）
- 非交互模式自动批准计划（`SRCODE_ALLOW_UNATTENDED_PLAN_APPROVAL`）
- LSP 自动格式化写回（`SRCODE_ALLOW_LSP_FORMAT_ON_WRITE`）
- 非交互模式运行项目代理（`SRCODE_ALLOW_UNATTENDED_PROJECT_AGENTS`）
