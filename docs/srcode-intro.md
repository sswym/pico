...
它的全称是"pico 编程代理"——"代理"这个词听起来唬人，其实意思很简单：**它能代表你、替你执行任务**。你只需要说清楚想要什么，具体怎么做、中间要查哪些资料、改哪些文件，都由它自己决定。
...
pico 想做的事就是：**把"动手干"的部分包下来，你只负责"说清楚想要什么"和"最后把关"**。
...
普通聊天机器人是"聊完就忘"的——每次对话都是重新开始。pico 不是，它有一本**属于自己的长期笔记本**，存在你电脑本地的一个小文件里。
...
面对复杂的任务，一个人从头干到尾容易顾此失彼。pico 的做法是：**大任务拆小，交给不同专长的"小助手"（子代理）分头干，最后汇总**。
...
改代码是有风险的——改错了可能让整个项目跑不起来。所以 pico 有一个"**先看后动**"的计划模式，像个审批流程：
...
如果你只记住一件事，就记这张图——它把 pico 的完整工作模式浓缩在一起：
...
**一句话总结**：pico 是一位住在你电脑里、记性好、会分工、守规矩的 AI 编程助手——你说清想要什么，它负责把"查、想、改、验"的活儿干完，把时间还给你，把把关权留给你。
