...
flowchart TD
    ...
    MAIN --> REG[ExtensionRegistry<br/>28 个扩展工厂, 按序注册]
    REG --> E1[prompt 层<br/>vibe / auto-thinking / ponytail / cache-optimizer / language]
...
### 4.19 第十九轮整改（2026-08-16，logo 紧凑阈值放宽：非最大窗口也显示完整 logo，1336 用例全绿）

**背景**：用户反馈终端非最大窗口（如 80×30）时 logo 只显示 2 行紧凑版（`✻ pico v0.1.0`），最大化才显示完整盒子。

**根因**：紧凑降级阈值过保守——宽度 `<72` 或高度 `<38` 就降级。实测盒子 `boxWidth = min(width-1, 98)` 自适应,40 列起即可完整渲染且不折行;高度 38 是十七轮针对「scrollview(follow:end) 滚动到底时 header 顶部滚出视野」设的保守值,但 30 行窗口 scrollview 通常仍能完整容纳 16 行盒子。

**改动**（`src/extensions/logo/index.ts`）：阈值 72/38 → 44/30。44 列起显示完整盒子（40 列即可渲染,44 留滚动条余量）;高度 ≥30 行显示完整盒子（<30 保留 2 行紧凑兜底,防止极端矮窗口盒子被裁）。
