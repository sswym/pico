...
不写 AGENTS.md、不初始化记忆库。integrations 节可选用**托管方式**下载 codegraph/rtk 到 `$PICO_HOME/bin/`（不污染系统 PATH；外部安装的副本优先于托管副本）、对当前项目执行 `codegraph init` 建索引、注册 CodeGraph MCP，并可开关自进化（evolution）扩展。
...
- 会话回退:undo 先 `waitForIdle` 等 agent 空闲,再导航会话树到该操作所属回合的 **user 消息**(`findUndoTurnUser` 沿 parent 链向上定位;多工具回合的 toolCall 消息父是前一个 toolResult,不能直接回退到父),整轮 bash/write/edit 卡从对话消失、输入框回到用户消息;redo 导航到编辑完成后的叶节点;非交互模式(如 headless)自动降级为纯文件回退。
