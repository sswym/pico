...
**验证**：手动测试每个 action 在 pico 自身项目上的表现。
...
- 支持 `.pico/lsp.json` 项目级配置
- 支持 `~/.pico/lsp.json` 用户级配置
...
**验证**：在 pico 项目（TypeScript）和一个 Rust 项目上测试多服务器路由。
...
**验证**：在 pico 项目上执行一次真实编辑，验证自动诊断返回。
