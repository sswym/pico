 ...
 *  long-term memory as "insight" facts.
 */
const INSTRUCTION_PATTERNS = [
  ...
  // ---- 一次性任务指令（祈使句），不是持久事实 ----
  // "在 <路径> 运行/执行 <命令>" / "运行 <命令>"
  /^\s*(?:请\s*)?(?:在\s*[^\s，。:：]{1,60}\s*中?)?\s*(?:运行|执行|启动|安装)\s+\S+/,
  // "把 <文件> 改成/删掉 <…>"
  /^\s*(?:请\s*)?(?:把|将)\s+[^\s，。]{1,80}(?:\s*的\s*)?(?:内容|文件|代码)?\s*(?:改成|改为|修改成|替换成|删掉|删除|移除|移到|移动)/,
  // "解释 <X> 如何/怎么 <Y>"
  /^\s*(?:请\s*)?(?:解释|说明|介绍一下|分析|解读|讲讲)\s+.+(?:如何|怎么|怎样|是什么|工作原理)/,
  // "数一下 / 列出 / 查一下 / 检查一下 …"
  /^\s*(?:请\s*)?(?:数一下|数数|统计一下|列出|找出|搜一下|搜索|查一下|看看|检查一下|确认一下)\s+/,
  // "优化/重构 <X>"
  /^\s*(?:请\s*)?(?:优化|改进|重构)(?:一下|下|了)?\s*\S+/,
  // "新增/修复 <X> …然后(运行|验证|提交|告诉我)"
  /^\s*(?:请\s*)?(?:新增|创建|添加|补上|删掉|删除|移除|修复|实现|完成)(?:一下|下|了)?\s+\S+.{0,80}?(?:然后|并|再|同时|，|。)?\s*(?:运行|测试|验证|提交|告诉我|输出)/,
];
